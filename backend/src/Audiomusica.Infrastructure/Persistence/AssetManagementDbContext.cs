...
public class AssetManagementDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Licencia>().HasKey(x => x.IdLicencia);
        modelBuilder.Entity<UsuarioLicencia>().HasKey(x => x.IdUsuarioLicencia);
        ...
        modelBuilder.Entity<UsuarioLicencia>().HasOne(x => x.Usuario).WithMany(x => x.UsuarioLicencias).HasForeignKey(x => x.IdUsuario).OnDelete(DeleteBehavior.Cascade);
        modelBuilder.Entity<UsuarioLicencia>().HasOne(x => x.Licencia).WithMany().HasForeignKey(x => x.IdLicencia).OnDelete(DeleteBehavior.Restrict);
        modelBuilder.Entity<UsuarioLicencia>().HasIndex(x => new { x.IdUsuario, x.IdLicencia }).IsUnique();
    ...
    private static void Seed(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Dimension>().HasData(
            new Dimension { IdDimension = 1, NumeroDimension = "DIM-001", NombreDimension = "TI Soporte", Pais = "Chile", Area = "TI", SubArea = "TI Soporte", Activo = true },
            new Dimension { IdDimension = 2, NumeroDimension = "DIM-002", NombreDimension = "Contabilidad", Pais = "Chile", Area = "Finanzas", SubArea = "Contabilidad", Activo = true },
            new Dimension { IdDimension = 3, NumeroDimension = "DIM-003", NombreDimension = "Bodega Central", Pais = "Chile", Area = "Logistica", SubArea = "Bodega Central", Activo = true },
            new Dimension { IdDimension = 4, NumeroDimension = "DIM-004", NombreDimension = "Tienda Trebol", Pais = "Peru", Area = "Retail", SubArea = "Tienda Trebol", Activo = true });
        ...
        modelBuilder.Entity<Licencia>().HasData(
            new Licencia { IdLicencia = 1, NombreLicencia = "Office" },
            new Licencia { IdLicencia = 2, NombreLicencia = "Microsoft" },
            new Licencia { IdLicencia = 3, NombreLicencia = "Adobe" },
            new Licencia { IdLicencia = 4, NombreLicencia = "SAP" },
            new Licencia { IdLicencia = 5, NombreLicencia = "TPV" },
            new Licencia { IdLicencia = 6, NombreLicencia = "VPN" },
            new Licencia { IdLicencia = 7, NombreLicencia = "Correo" },
            new Licencia { IdLicencia = 8, NombreLicencia = "TeamViewer" });
        ...
        modelBuilder.Entity<Usuario>().HasData(
            new Usuario { IdUsuario = 1, Rut = "11111111-1", NombreUsuario = "Usuario", ApellidoPaterno = "Admin", CorreoCorporativo = "admin@audiomusica.cl", Cargo = "Jefe TI", FechaIngreso = new DateTime(2026, 1, 1), IdRol = 1, IdDimension = 1, Activo = true, IdCuenta = 5 },
            new Usuario { IdUsuario = 2, Rut = "22222222-2", NombreUsuario = "Usuario", ApellidoPaterno = "Normal", CorreoCorporativo = "normal@audiomusica.cl", Cargo = "Analista", FechaIngreso = new DateTime(2026, 1, 1), IdRol = 2, IdDimension = 1, Activo = true, IdCuenta = 1 });
        modelBuilder.Entity<UsuarioLicencia>().HasData(
            new UsuarioLicencia { IdUsuarioLicencia = 1, IdUsuario = 1, IdLicencia = 7 },
            new UsuarioLicencia { IdUsuarioLicencia = 2, IdUsuario = 2, IdLicencia = 1 });
